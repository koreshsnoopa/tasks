...
namespace exceptions_task
{
    public class Chassis
    {
        ...
        public Chassis(int numberOfWheels, string numberOfChassis, double permissibleLoad)
        {
            ...
            {
                throw new InitializationException("Number of wheels must be more then 0!");
            }

            if (String.IsNullOrEmpty(numberOfChassis))
            {
                throw new InitializationException("You need to enter number of chassis!");
            }
            else
            {
            ...
            {
                throw new InitializationException("Permissible load must be more then 0!");
            }
        }
